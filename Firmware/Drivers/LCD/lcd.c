...
/* Idle callback — ticks the PD stack while waiting for SPI DMA completion.
 * Without this, the ~15ms DMA transfer blocks axxpd_run() and can cause
 * the PD source to Hard Reset (missed tSenderResponse / EPR KeepAlive). */
static LCD_IdleCallback lcd_idle_cb = NULL;

void LCD_SetIdleCallback(LCD_IdleCallback cb) { lcd_idle_cb = cb; }
...
static void setSPI_Size(int8_t size){
  if(config.spi_sz!=size){
    ...
    /* Re-initialize SPI to actually program CR2 with the new data size.
     * Without this, the SPI hardware stays in 8-bit mode and 16-bit
     * DR writes come out byte-swapped (low byte first on ARM LE). */
