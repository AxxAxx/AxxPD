...
/* Pin connections — portable macros (port + pin) used by lcd.c */
...
#define LCD_CS                /* Enable CS toggling in lcd.c */
#define LCD_RST               /* Enable RST pulse in lcd.c   */
...
#define LCD_ROTATION 3                /* Landscape for 1.47" panel. Valid values: 0...3 */
...
//#define LCD_240X280
//#define LCD_240X320
#define LCD_172X320                   /* 1.47" IPS panel (172 visible rows in 240-row buffer) */
#endif
  ...
  /* RGB MADCTL: with SPI CR2 fix, standard RGB565 displays correctly. */
  #if LCD_ROTATION == 0
    #define LCD_ROTATION_CMD (CMD_MADCTL_MX | CMD_MADCTL_MY | CMD_MADCTL_RGB)
    #ifdef LCD_135X240
...
/* Standard RGB565: R[15:11](5) G[10:5](6) B[4:0](5).
 * setSPI_Size() now writes to SPI CR2, so 16-bit transfers send MSB first. */
#define color565(r, g, b) (((r & 0xF8) << 8) | ((g & 0xFC) << 3) | ((b & 0xF8) >> 3))
#define RGB(r, g, b) color565(r, g, b)

/* LEGACY(r,g,b) — reproduces the old visual appearance from the previous
 * firmware where a BRG macro + byte-swapped SPI created a specific look.
 * Use for existing UI colors. Use RGB() for all NEW colors. */
#define LEGACY(r, g, b) __builtin_bswap16( \
    (((uint16_t)((b) & 0xF8)) << 8) | (((uint16_t)((r) & 0xFC)) << 3) | (((uint16_t)((g) & 0xF8)) >> 3))

#define ABS(x) ((x) > 0 ? (x) : -(x))

/* Helpers for direct BSRR register access (fast GPIO) */
#define LCD_SET_DC()   (LCD_DC_PORT->BSRR = LCD_DC_PIN)
#define LCD_CLR_DC()   (LCD_DC_PORT->BSRR = (uint32_t)LCD_DC_PIN << 16u)
#define LCD_SET_CS()   (LCD_CS_PORT->BSRR = LCD_CS_PIN)
#define LCD_CLR_CS()   (LCD_CS_PORT->BSRR = (uint32_t)LCD_CS_PIN << 16u)
#define LCD_SET_RST()  (LCD_RST_PORT->BSRR = LCD_RST_PIN)
#define LCD_CLR_RST()  (LCD_RST_PORT->BSRR = (uint32_t)LCD_RST_PIN << 16u)

/* Idle callback — called repeatedly while waiting for DMA transfers.
 * Set this to axxpd_run() so the PD stack keeps ticking during LCD updates. */
typedef void (*LCD_IdleCallback)(void);
void LCD_SetIdleCallback(LCD_IdleCallback cb);
