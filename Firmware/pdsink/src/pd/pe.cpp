...
public:
    static auto on_enter_state(PE& pe) -> state_id_t {
        ...
        if (!rdo_and_pdo.first) {
            ...
            // Don't HR on empty RDO — just go back and wait for next SRC_CAPA.
            // This happens on the first negotiation when source_caps isn't
            // populated yet due to tick ordering (PE runs before PRL).
            return PE_SNK_Wait_for_Capabilities;
        }
...
public:
    static auto on_enter_state(PE& pe) -> state_id_t {
        ...
        // Suppress EPR auto-re-entry after Hard Reset.  Some chargers
        // falsely advertise EPR capability (bit 23) but reject EPR entry.
        // Without this, clear_all() wipes EPR_AUTO_ENTER_DISABLED and the
        // PE loops: Ready → EPR entry → fail → Soft Reset → Ready → ...
        // The application (boot_selector, cli) clears this flag when it
        // explicitly requests EPR.
...
public:
    ...
    static auto on_run_state(PE& pe) -> state_id_t {
        ...
        {
            // Suppress auto-re-entry after timeout — chargers that falsely
            // advertise EPR will time out here on every attempt.
            port.pe_flags.set(PE_FLAG::EPR_AUTO_ENTER_DISABLED);
            port.dpm_requests.clear(DPM_REQUEST_FLAG::EPR_MODE_ENTRY);
            return PE_SNK_Send_Soft_Reset;
        }
    ...
    static void on_exit_state(PE& pe) {
        // Always stop tEnterEPR on exit — whether transitioning to
        // Wait_For_Response (normal) or Soft_Reset (protocol error).
        // Leaving it running across state transitions causes spurious
        // timeouts in the next state.
...
void PE_EventListener::on_receive(const MsgToPe_PrlMessageReceived&) {
    ...
    // DEBUG: store state + message type for external debug dump
    static volatile uint16_t pe_rx_state = 0;
    static volatile uint16_t pe_rx_hdr = 0;
    static volatile uint32_t pe_rx_count = 0;
    pe_rx_state = pe.get_state_id();
    pe_rx_hdr = pe.port.rx_emsg.header.raw_value;
    pe_rx_count++;
    (void)pe_rx_state; (void)pe_rx_hdr; (void)pe_rx_count;
}
