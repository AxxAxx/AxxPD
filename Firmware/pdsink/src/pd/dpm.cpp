...
auto DPM::get_sink_pdo_list() -> PDO_LIST {
    ...
    set_snk_pdo_limits(pdo11.raw_value,
        PDO_LIMITS().set_mv_min(15000).set_mv_max(48000).set_pdp(get_epr_watts()));
...
void DPM::fill_rdo_flags(uint32_t &rdo) {
    ...
    // Mirror source PDO1 EPR bit — setting epr_capable=1 when the source
    // has EPR=0 causes a Hard Reset on strict chargers (PD 3.1 s6.4.2.4.3).
    //
    // Per USB-PD 3.1 Table 6.9 (Fixed Supply PDO – Source), bit 23 is
    // "EPR Mode Capable" (bit 22 is reserved). pdsink upstream incorrectly
    // extracts bit 22 here; use bit 23. The sink RDO's epr_capable bit is
    // at 22 (Table 6.23), so the union write below targets the correct bit.
    //
    // NOTE: we always mirror the source's bit, even when the sink doesn't
    // intend to enter EPR. Observed Anker A2697 behaviour: if our SPR
    // Request for the 20 V/5 A PDO sets epr_capable=0, Anker repeatedly
    // re-advertises Source_Capabilities instead of sending PS_RDY, in an
    // endless livelock. Declaring EPR capability truthfully (sink IS
    // EPR-capable; just not entering right now) satisfies the source.
    rdo_bits.epr_capable = (port.source_caps.size() > 0 &&
                            ((port.source_caps[0] >> 23) & 1)) ? 1 : 0;
